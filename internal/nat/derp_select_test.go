...
func TestPickNearestDERPNodeBreaksLatencyTiesByRegionID(t *testing.T) {
	derpMap := &vderp.Map{
		Regions: map[int]vderp.Region{
			5: {
				RegionID: 5,
	...
	originalProbe := measureDERPNodeLatencyFunc
	measureDERPNodeLatencyFunc = func(_ vderp.Node, _ time.Duration) time.Duration {
